...
See `test/cookbooks/test/recipes/` for many more examples of `apache2_module`.

## Removal

`:delete`: Removes the module .conf and .load files and their enabled links. For built-in module configuration, deletion also invokes the matching module resource cleanup. It does not uninstall externally managed packages or remove external module binaries. Use the package-specific resource to remove a packaged module.

```ruby
apache2_module 'default' do
  action :delete
end
```
...
| --- | --- | --- | --- |
| `path` | String | `platform module directory + mod_name` | Full path to the module shared object. |
| `apache_service_notification` | Symbol | `:reload` | Reserved notification preference; notify apache2_service explicitly. |
