...
`:remove`: Stops and disables Apache, removes its server and Perl packages, and recursively removes Apache configuration, logs, cache and lock directories, plus the installed management scripts. This deletes locally added sites and logs. Back up anything you need before invoking it. The document root and shared users/groups are retained.
...
| --- | --- | --- | --- |
| `apache_pkg` | String | `platform-specific` | Name of the server package. |
| `apache_version` | String | `''` | Version of the server package; empty selects the latest available. |
