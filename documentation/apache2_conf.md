...
| options           | Hash   | server_tokens: 'Prod', server_signature: 'Off', trace_enable: 'Off', | Hash of key-value pairs to pass to the template (useful for overridden templates)  |
...
`:delete`: Removes the generated configuration file and enabled link.

```ruby
apache2_conf 'default' do
  action :delete
end
```
