...
describe 'apache2::mod_include' do
  describe file("#{property[:apache][:dir]}/mods-available/include.load") do
    it { should be_file }
    it { should be_mode 644 }
    its(:content) { should match "LoadModule include_module #{property[:apache][:libexec_dir]}/mod_include.so\n" }
  end

  describe file("#{property[:apache][:dir]}/mods-enabled/include.load") do
    it { should be_linked_to "#{property[:apache][:dir]}/mods-available/include.load" }
  end
